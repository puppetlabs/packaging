...
describe 'Pkg::Deb::Repo' do
  ...
  let(:repo_configs)  { cows.reject {|cow| cow.empty?}
                          .map {|dist| "pkg/repo_configs/deb/pl-#{project}-#{ref}-#{dist}.list" } }
  ...
  describe '#generate_repo_configs' do
    it 'fails if wget isn\'t available' do
      expect(Pkg::Util::Tool)
        ...
        .with('wget', {:required => true}).and_raise(RuntimeError)
    ...
    it 'warns if there are no deb repos available for the build' do
      ...
      expect(Pkg::Util::Execution)
        .to receive(:capture3)
              .with("#{wget} --no-verbose --spider --recursive --level=1 " \
                    "--no-parent #{base_url}/repos/apt/ 2>&1")
    ...
    it 'writes the expected repo configs to disk' do
      expect(Pkg::Util::Execution)
        .to receive(:capture3)
              .with(/.*\/wget --no-verbose --spider --recursive --level=1 --no-parent #{base_url}\/repos\/apt\/ 2>&1/)
  ...
  describe '#retrieve_repo_configs' do
    ...
    it 'warns if there are no deb repos available for the build' do
      ...
      expect(Pkg::Util::Execution)
        .to receive(:capture3)
              .with("#{wget} --no-verbose --recursive --no-parent --no-host-directories --cut-dirs=3 --directory-prefix=pkg/repo_configs --reject 'index*' #{base_url}/repo_configs/deb/")
              .and_raise(RuntimeError)
  ...
  describe "#create_repos" do
    ...
    it "generates repo configs remotely and then ships them" do
      ...
      expect(Pkg::Util::Net).to receive(:remote_execute)
  ...
  describe '#sign_repos' do
    ...
    it "makes a repo for each dist" do
      ...
      distfiles = {}
      expect(Pkg::Util::File).to receive(:directories).with('repos/apt').and_return(dists)
