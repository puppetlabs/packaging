namespace :pl do
  namespace :remote do
    ...
    task :update_yum_repo => 'pl:fetch' do
      yum_whitelist = {
        ...
        __GPG_KEY__: Pkg::Util::Gpg.get_key,
      }
  ...
  task :ship_check => 'pl:fetch' do
    ...
    gpg_errs << Pkg::Util::Net.check_host_gpg(Pkg::Config.apt_signing_server, Pkg::Util::Gpg.get_key)
    gpg_errs << Pkg::Util::Net.check_host_gpg(Pkg::Config.distribution_server, Pkg::Util::Gpg.get_key)
    gpg_errs.flatten!
    # ignore gpg errors for hosts we couldn't ssh into
    gpg_errs -= ssh_errs
    unless gpg_errs.empty?
      gpg_errs.each do |host|
        errs << "Secret key #{Pkg::Util::Gpg.get_key} not found on #{host}"
      end
    end
